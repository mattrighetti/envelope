...
/// Detects the current state of the envelope file and opens it.
pub async fn detect() -> Result<Option<EnvelopeState>> {
    ...
    if bytes_read >= SQLITE_MAGIC.len() && buf[..SQLITE_MAGIC.len()] == *SQLITE_MAGIC {
        ...
        return Ok(Some(EnvelopeState::Unlocked(envelope)));
    }
